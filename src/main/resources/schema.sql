...
(
    ...
    email    VARCHAR NOT NULL CHECK (LENGTH(TRIM(email)) > 0),
    login    VARCHAR UNIQUE NOT NULL CHECK (LENGTH(TRIM(login)) > 0),
    name     VARCHAR,
    birthday DATE
);
...
(
    ...
    name VARCHAR UNIQUE NOT NULL CHECK (LENGTH(TRIM(name)) > 0)
);
